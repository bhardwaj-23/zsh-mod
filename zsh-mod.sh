...
read -p "Enter the username to configure ZSH for (e.g., Joe, Piyush): " TARGET_USER
...
cd "$TEMP_DIR"
FASTFETCH_URL="https://github.com/fastfetch-cli/fastfetch/releases/download/2.58.0/fastfetch-linux-amd64.deb"
wget -4 -q --show-progress -O fastfetch.deb "$FASTFETCH_URL"

if [ -f "fastfetch.deb" ]; then
    apt install -y ./fastfetch.deb
else
    printf "${RED}Failed to download Fastfetch.${NC}\n"
fi
